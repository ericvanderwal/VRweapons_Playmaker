...
namespace HutongGames.PlayMaker.Actions
{
	...
	[Tooltip("Adjust bullet spread max for the VRweapons script on an object")]

	// the class must match the name of the action
	// if the action is named missleAction then that should be the name of the class
	public class WeaponBulletSpreadMaxAction  : FsmStateAction
	{
		[RequiredField]
		...
		[Tooltip("Set the bullet spread max. Max 0.2")]
		[HasFloatSliderAttribute(0, 0.2f)]
		public FsmFloat bulletSpreadMax;
		...
		void MakeItSo()
		{
			...
			theScript.bulletSpreadMax = bulletSpreadMax.Value;
